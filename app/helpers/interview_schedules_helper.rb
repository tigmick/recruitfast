    ...
    html = ""
			...
			unless schedule.client_comments.empty?
			  schedule.client_comments.each do |comment|
			   html += "<div><b>#{comment.user.first_name}</b> : <a href='#' data-toggle='tooltip' data-placement='bottom' title='#{comment.comment}'>#{comment.comment.truncate(55).capitalize}</a></div>"
	  ...
	  html = ""
		  unless schedule.candidate_feedbacks.empty?
		    schedule.candidate_feedbacks.each do |feedback|
		     html +=  "<div><b>#{feedback.user.first_name}</b> : <a href='#' data-toggle='tooltip' data-placement='bottom' title='#{feedback.feedback}'>#{feedback.feedback.truncate(55).capitalize}</a></div>"
